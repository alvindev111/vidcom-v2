...
CREATE TABLE `__new_mutation_step` (
	...
	CONSTRAINT "ck_step_capture_shape" CHECK(("capture_state" = 'pending' OR (("previous_content" IS NULL AND "captured_hash" IS NULL) OR ("previous_content" IS NOT NULL AND "captured_hash" IS "from_hash")))),
...
INSERT INTO `__new_mutation_step`(`id`, `journal_id`, `ordinal`, `kind`, `path`, `entity`, `from_hash`, `to_hash`, `previous_content`, `previous_byte_size`, `status`) SELECT `id`, `journal_id`, `ordinal`, `kind`, `path`, `entity`, `from_hash`, `to_hash`, `previous_content`, `previous_byte_size`, `status` FROM `mutation_step`;--> statement-breakpoint
