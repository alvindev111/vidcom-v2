...
export function setPreviewSettingsTool(
  ...
  return {
    ...
    description: [
      ...
      "Preconditions: projectId and expectedRevision come from get_project_context.project.id and get_project_context.previewSettingsRevision; a bgm.track path must name an existing mp3, wav, ogg or m4a asset listed by list_project_assets, and setting bgm.track to null detaches the music.",
      "Side effects: merges the patch into preview-settings.json as one journaled entity mutation, returning the complete settings, their entity revision and the new projectRevision; a palette id applies all semantic colors atomically, while an individual color override clears the preset id.",
