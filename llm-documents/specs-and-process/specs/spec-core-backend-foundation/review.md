...
| Real Next/SSE/runtime milestone | Resolved locally | CI chạy `scripts/verify-next-runtime.mjs` sau build qua `next start` thật: nonce exchange, workspace list, watcher, no-buffer SSE và reconnect `Last-Event-ID`. Local run xanh; còn chờ commit/push và remote CI của phiên này trước khi đóng spec. |
