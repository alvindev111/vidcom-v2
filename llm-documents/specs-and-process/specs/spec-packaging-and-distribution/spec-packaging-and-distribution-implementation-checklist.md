...
- [ ] K.6 `POST /v1/projects/imports` trả **202 `{jobId}`** — **MỞ LẠI: endpoint chết trong mọi mode**
  ...
  - Để tick lại cần: `startProjectImport` mở một `workspace_operation` kind `project_import`, chạy staging + commit qua coordinator, rồi settle row đó — và trả id của operation làm `jobId`
