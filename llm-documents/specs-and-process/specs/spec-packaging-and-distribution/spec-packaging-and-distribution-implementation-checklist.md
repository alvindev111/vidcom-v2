...
> - [Main spec](./spec-packaging-and-distribution-complete.md)
...
- **Trạng thái thực thi**: **Hoàn tất 2026-08-16 — 13 phase A–M đóng, 174/174 mục**. Mục cuối cùng còn mở là Acceptance Criterion artifact của Phase D, đóng bằng C-71: nguồn release C-21 đã duyệt nhưng chưa có code chạy nó, nên [`build-release-media.mjs`](../../../../scripts/build-release-media.mjs) dựng ffmpeg/ffprobe 7.1.1 từ đúng các pin upstream và `prepare-packaged-runtime --release-media` chỉ nhận binary có provenance khớp cả nguồn lẫn bytes. Production `build:artifact --release --json` xanh ở commit `3f49e7c` (artifact SHA-256 `fb658b6b…56d7a1`, `ffmpeg 7.1.1`) và packaged smoke trên chính artifact đó ra MP4 8,000000 s `h264`+`aac` với PATH rỗng. Bước `offline` không chạy được trên máy dev vì đòi cắt mạng ở tầng runner; nó đã đóng ở M.4 bằng exact-head CI ba OS. Ba nền tảng native vẫn xanh ở exact head `a737de6`/`31710887527` với fixture runtime của CI. Main spec là [`-complete.md`](./spec-packaging-and-distribution-complete.md).
...
**Acceptance Criteria**:
- [x] Render MP4 **từ production release artifact** trên clean app-data/private PATH — **ĐÓNG 2026-08-16 (C-71)**: nguồn release của C-21 trước đây được duyệt nhưng **không có code nào chạy nó**, nên đường duy nhất tới artifact vẫn là smoke fixture. [`build-release-media.mjs`](../../../../scripts/build-release-media.mjs) dựng ffmpeg/ffprobe **7.1.1 từ đúng các pin upstream** trong `build-ffmpeg.mjs`, và `prepare-packaged-runtime --release-media` chỉ nhận binary khi provenance nêu đúng tập nguồn được duyệt **và** bytes cạnh nó vẫn khớp digest record đã đo — không có source build thì opt-in fixture vẫn từ chối
  - **Production artifact**: commit `3f49e7c9da7636ed360381fd39883497d84935ad`, `dirty=false`, `build:artifact --release --json` xanh; `artifactVersion 0.1.0`, `ffmpeg 7.1.1`, artifact SHA-256 `fb658b6b…56d7a1`. Runtime input `ffmpegSha256` **bằng đúng** giá trị provenance đo được ⇒ bytes trong artifact truy được về archive nguồn, không phải mirror
  - **Render**: packaged smoke trên artifact đó với PATH rỗng (`no node, python or bun on PATH`), app-data/runtime rỗng lúc bắt đầu — `render-media` xanh: VieNeu ra WAV, snapshot xanh, **MP4 8,000000 s, `ffprobe` đọc được stream `h264` + `aac`**, và post-media doctor `db.migration`/`runtime.integrity`/`chrome.cache`/`tts.model-cache` đều `ok`. `render-cli` cũng xanh gồm exhaustive zero-survivor cancellation proof
  - **12/13 bước xanh tại chỗ; `offline` không chạy được trên máy dev** vì nó đòi cắt mạng ở **tầng runner** (`sudo -n route`, không có passwordless sudo). Đây đúng là lý do M.4 tồn tại và nó đã đóng bằng exact-head CI ba OS; MUST NOT viết lại bước đó thành env-level để làm xanh cục bộ
  - Không nới `--release`: verify-artifact vẫn chỉ đòi clean tree + commit. CI packaged smoke **cố ý** vẫn dùng fixture, nên gate release là nguồn runtime được đưa vào build, không phải một cờ mới
  - **Đã bắt được hai lỗi thật khi đóng mục này, cả hai đã sửa (C-72)**: (1) FFmpeg nướng nguyên dòng configure vào binary, nên prefix build nằm trong checkout làm `verify-artifact` từ chối staged `ffmpeg` với `build-root`; prefix nay nằm ngoài checkout và `build-release-media` tự hỏi câu đó ngay với binary vừa ra khỏi compiler. (2) Harness ghi bằng chứng **sau** teardown, nên một lỗi persist cache xoá sạch report của 13 bước đã xong; teardown nay chạy sau evidence, write-back chịu lỗi theo entry và nêu tên trong `cacheWriteBack`, lỗi evidence thành `evidenceError` và vẫn làm lượt chạy đỏ
  - Lượt chạy xác nhận sau khi sửa: 12/13 xanh với `render-media` ra MP4 8,000000 s, report ghi `cacheWriteBack: []` và `evidenceError: null`; một lượt `--step build` cố tình dựng cache root hỏng cho ra report đầy đủ kèm đúng một entry write-back bị bỏ qua
...
> Con số này **phân hoạch lại** ước lượng theo R ở [main spec](./spec-packaging-and-distribution-complete.md) (R1 26 · R2 37 · R3 13 · R4 21 · R5 21 · R6 25 · R7 8 · R8 21 · R9 5), không phải một ước lượng thứ hai. Tổng giữ nguyên.
...
2026-08-16 — Phase D AC: release media source build C-71
  - Files: `scripts/build-release-media.mjs` (mới), `scripts/prepare-packaged-runtime.mjs`, `scripts/build-ffmpeg.mjs`, `tests/build/release-media.test.ts`, `package.json`, `tsconfig.json`, Detailed Design §16, checklist và implementation notes.
  - Summary: C-21 duyệt nguồn release từ Phase M nhưng chưa bao giờ có driver chạy nó, nên đường duy nhất tới artifact vẫn là smoke fixture của mirror bên thứ ba. Driver mới dựng ffmpeg/ffprobe 7.1.1 từ đúng các pin upstream, hỏi linker xem binary còn phụ thuộc gì máy sạch không có (chỉ `/usr/lib` + System frameworks), rồi ghi provenance nêu đích danh archive nguồn. `prepare-packaged-runtime --release-media` chỉ nhận binary khi record nêu đúng tập nguồn được duyệt **và** bytes vẫn khớp digest record đã đo. Production `build:artifact --release --json` xanh ở commit `3f49e7c`, artifact SHA-256 `fb658b6b…56d7a1`, `ffmpeg 7.1.1`; packaged smoke trên chính artifact đó ra **MP4 8,000000 s có video + audio** với PATH rỗng và app-data/runtime rỗng.
  - Decisions: Xin release media không phải là quyền — không có source build thì opt-in fixture vẫn từ chối, nên đường mới không thể dùng để đi vòng qua gate. Không nới `--release` của verify-artifact: CI packaged smoke cố ý vẫn dùng fixture, gate release là nguồn runtime đưa vào build. x264 host trả `406` cho HTTP client của Node và `200` cho curl cùng URL ⇒ fallback transport thứ hai, digest vẫn là thứ quyết định bytes. Prefix build chuyển ra ngoài checkout vì FFmpeg nướng nguyên dòng configure vào binary và `verify-artifact` từ chối đúng theo luật `build-root`.
  - Blockers: `offline` là bước duy nhất không chạy được cục bộ — nó đòi cắt mạng ở tầng runner (`sudo -n route`) và máy dev không có passwordless sudo; bước này đã đóng ở M.4 bằng exact-head CI ba OS và MUST NOT hạ xuống env-level để xanh cục bộ. Nguồn release cho Linux/Windows chưa dựng: driver chạy trên host nào thì pin/verify cho host đó, và DR-1 vẫn cấm cross-build.

2026-08-16 — Harness evidence survival + compiler-time build-root guard C-72
  - Files: `scripts/packaged-smoke/run.mjs`, `scripts/packaged-smoke/environment.mjs`, `scripts/build-release-media.mjs`, `tests/build/packaged-smoke.test.ts`, `tests/build/release-media.test.ts`, Detailed Design §16, checklist và implementation notes.
  - Summary: Lượt release thật hoàn tất 13 bước rồi mất sạch report + evidence bundle vì `dispose()` chạy **trước** khi ghi bằng chứng và `cp` ném lúc persist cache. Teardown nay chạy sau khi evidence đã nằm trên đĩa; write-back chịu lỗi theo từng entry và nêu tên entry trong report (`cacheWriteBack`); lỗi evidence được giữ lại thành `evidenceError` thay vì ném. Chiều seed cache vẫn ném vì đó là setup mà bước sau khẳng định dựa lên. Song song, `build-release-media` tự hỏi binary vừa ra khỏi compiler có mang đường checkout không — cùng câu hỏi `verify-artifact` đặt, nhưng sớm hơn một runtime stage + archive + SEA build.
  - Decisions: Lỗi write-back **không** đổi exit code (nó chỉ tốn một lần tải lại ở lượt sau) nhưng phải xuất hiện trong report và trên stderr; lỗi evidence **có** làm lượt chạy đỏ. Smoke root vẫn bị xoá kể cả khi write-back dở dang — vài trăm MB mỗi lượt, và entry hỏng đã được nêu tên thay vì để lại cho người khác đoán.
  - Blockers: Không có; `tests/build/packaged-smoke.test.ts` 36/36, `tests/build/release-media.test.ts` 13/13, typecheck và lint xanh.
