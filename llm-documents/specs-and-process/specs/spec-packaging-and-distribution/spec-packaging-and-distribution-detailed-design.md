...
| C-68 | Sau khi `render --detach` trả job ID, 120 s luôn đủ để job Windows tới supervised spawn | Exact Windows attempt 2 không còn chết ở outer enqueue 120 s: detach trả job ID nhưng observation 120 s hết trước stage `rendering video`, trong khi cùng runner media pipelines mất 344–404 s. Observation budget riêng là 360 s, không đổi product enqueue/process timeout và không retry mutation; timeout failure ghi last status/stage để phân biệt queued/preparing/worker failure | checklist M.3c/M.9, `scripts/packaged-smoke/bodies.mjs`, `tests/build/packaged-smoke.test.ts` |
