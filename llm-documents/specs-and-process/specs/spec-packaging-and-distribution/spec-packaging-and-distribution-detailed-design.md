...
> Bản 2 vẫn **APPROVED**. Phụ lục này không mở lại phạm vi, không thêm/bớt requirement và **không đổi ước lượng 177 SP**. Nó sửa ba mươi sáu chỗ mà bản 2 nói khác code, manifest authority hoặc hành vi runtime đã đo, phát hiện khi review và thực thi Implementation Checklist. Mỗi món nêu bằng chứng đã kiểm để không phải kiểm lại.
...
| C-33 | HyperFrames exit `0` đủ để chuyển sang ffprobe; config/telemetry người dùng được phép tự bật experimental parallel router | Exact pin có thể tự arm de-parallel router từ mutable telemetry/config và exit `0` mà không tạo output. Production ép `HF_DE_PARALLEL_ROUTER=false`; sau exit `0` phải lấy regular-file/hash `artifactSource` trước ffprobe, thiếu output fail bằng lỗi riêng, và publication dùng lại chính source đã chứng minh | checklist M.3c/M.4, `packages/worker/src/render-job.ts`, `tests/adapter/render-job.test.ts` |
