...
> Bản 2 vẫn **APPROVED**. Phụ lục này không mở lại phạm vi, không thêm/bớt requirement và **không đổi ước lượng 177 SP**. Nó sửa tám chỗ mà bản 2 nói khác code hoặc contract đã phát hành trong repo, phát hiện khi review và thực thi Implementation Checklist. Mỗi món nêu bằng chứng đã kiểm để không phải kiểm lại.
...
**Không có món nào trong tám món này làm thay đổi kiến trúc.** C-1 là món duy nhất đổi *chỗ đặt code* (remote `ToolInvoker`: `mcp` → `cli`), và nó đổi theo hướng **thắt lại**, không nới. C-6 đóng băng vocabulary của một wire contract đã phát hành; nó không bỏ mã lỗi khỏi các boundary packaging cần chúng. C-8 giữ nguyên thứ tự cold boot của artifact, chỉ làm rõ cách source-dev đi qua cùng composition root khi chưa có archive để extract.
