...
**Không có món nào trong bảy món này làm thay đổi kiến trúc.** C-1 là món duy nhất đổi *chỗ đặt code* (remote `ToolInvoker`: `mcp` → `cli`), và nó đổi theo hướng **thắt lại**, không nới. C-6 đóng băng vocabulary của một wire contract đã phát hành; nó không bỏ mã lỗi khỏi các boundary packaging cần chúng.
