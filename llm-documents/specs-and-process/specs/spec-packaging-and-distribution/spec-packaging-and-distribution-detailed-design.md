...
Trần cold giữ từ topology đã đo: SEA base 116 MB, archive Python 146 MB nén → 492 MB giải nén (§5.13). Warm boot hiện authenticate generation khoảng 638 MB/8067 file ở outer SEA rồi inner CLI bootstrap; baseline cũ `0fdbd35` có trước topology này nên không còn đại diện. Baseline v2 bind cohort `double-deep-integrity-v1` và lưu tối thiểu 5 raw independent hosted-runner sample/OS. Gate derive center=median, empirical upper=nearest-rank p95 và limit=`max(1,5×median, upper+200 ms)`; hard warm ceiling độc lập là macOS **8 s**, Linux **11 s**, Windows **20 s**, đều cao hơn statistical limit ít nhất polling quantum. Cold ceiling vẫn **120/120/180 s**. Warm-app **2/2/3 s** giữ nguyên vì chưa có measurement authority tương đương trong M.3a.
...
| C-49 | Baseline thiếu hoặc sai schema có thể được coi như runner lần đầu và chỉ báo `baselinePresent=false` | Sau khi ba runner đã có evidence, baseline là release gate đã commit chứ không còn là optional capture. Reader chỉ chấp nhận schema đóng, exact runner và complete sample set; packaged smoke fail nếu file thiếu/hỏng/sai runner. Nhờ vậy xoá hoặc làm hỏng file trong PR không thể âm thầm tắt gate thống kê | checklist H.6/M.7, `.github/perf-baseline/*.json`, `scripts/{measure-startup,packaged-smoke/bodies}.mjs`, `tests/build/startup-baseline.test.ts` |
...
| C-65 | Một workload deadline 120 s đủ cho cả tool, render enqueue và mọi lần đọc trạng thái job | Exact Windows packaged smoke `8ee68e4` chạy render-media/offline thành công nhưng CLI render wait ném `internal_error` sau 141,968 ms; tổng thời gian khớp SEA/deep bootstrap cộng preflight enqueue chạm trần 120 s. Render enqueue có deadline riêng 300 s và tuyệt đối không retry vì POST có thể đã được áp dụng. `GET /jobs/:id` dùng deadline 120 s và retry đúng một lần chỉ khi fetch/abort chưa nhận HTTP response; structured HTTP error và mọi mutation vẫn không retry. CLI chỉ in stable redacted error code; smoke failure ghi phase/timing, daemon state/tail đã redact và latest render row read-only | checklist J/M, `packages/adapter/src/daemon/daemon-client.ts`, `packages/cli/src/commands/render.ts`, `scripts/packaged-smoke/bodies.mjs`, focused daemon/render/smoke tests |
| C-66 | Detached render smoke có thể giữ outer process timeout 120 s sau khi product enqueue budget tăng lên 300 s | Exact Windows `a05a0ff` chứng minh render wait đầu tiên đã hoàn tất, nhưng lệnh `render --detach` bị harness `spawnSync` kill ở 120 s; tổng step 438,221 ms bằng render đầu khoảng 318 s cộng detached ceiling. Outer detached budget là 360 s = product enqueue 300 s + 60 s SEA startup/Windows AV margin. Mutation vẫn gọi đúng một lần, không retry; detached failure đi qua cùng redacted phase/timing/daemon/latest-job diagnostic | checklist M.3c/M.9, `scripts/packaged-smoke/bodies.mjs`, `tests/build/packaged-smoke.test.ts` |
| C-67 | Một exact hosted-runner sample có thể đại diện phân phối startup và hard ceiling nên nằm sát `1,5×` sample đó | Hai macOS run đầu vượt v1 warm threshold chỉ `26 ms` và `2 ms`, đều dưới readiness polling quantum `200 ms`; attempt độc lập kế tiếp đo `7.091 s`, chứng minh distribution rộng hơn một sample chứ không biện minh nudge threshold. Baseline v2 lưu tối thiểu 5 raw sample độc lập/OS với exact commit + evidence duy nhất và bind reviewed performance cohort; derive median, empirical nearest-rank p95 và regression limit `max(1,5×median, p95 + 200 ms)`. Missing/duplicate/non-safe-integer/wrong-cohort/extra-key fail closed; hard ceiling cold/warm phải cao hơn statistical limit ít nhất một measurement quantum | checklist M.7, `.github/perf-baseline/*.json`, `scripts/measure-startup.mjs`, `tests/build/startup-baseline.test.ts` |
| C-68 | Sau khi `render --detach` trả job ID, 120 s luôn đủ để job Windows tới supervised spawn | Exact Windows attempt 2 không còn chết ở outer enqueue 120 s: detach trả job ID nhưng observation 120 s hết trước stage `rendering video`, trong khi cùng runner media pipelines mất 344–404 s. Observation budget riêng là 360 s, không đổi product enqueue/process timeout và không retry mutation. Sau khi có job ID, mọi failure path best-effort cancel đúng một lần, boundedly thu terminal + termination proof/cleanup state và giữ lỗi observation làm primary; diagnostic chỉ xuất status/stage allowlisted, elapsed và daemon tail đã redact | checklist M.3c/M.9, `scripts/packaged-smoke/bodies.mjs`, `tests/build/packaged-smoke.test.ts` |
| C-69 | MCP stdio chỉ hợp lệ khi child không ghi byte nào lên stderr | MCP dành stdout cho JSON-RPC nhưng stderr là diagnostic channel hợp lệ. Exact run `55be637` đã hoàn tất legacy/modern initialize, tools/read/write và single-lease coexistence trên ba OS rồi bị harness làm đỏ chỉ vì stderr khác rỗng lúc close. Smoke nay drain stderr để tránh backpressure nhưng không buffer/copy nội dung riêng tư vào evidence; protocol/call/close failure thật vẫn fail theo phase và được redact | checklist I.9/M.3b, `scripts/packaged-smoke/bodies.mjs`, `tests/build/packaged-smoke.test.ts` |
| C-70 | Một real-filesystem matrix nhiều candidate phải dùng global test timeout 5 giây như unit case đơn | Exact Linux CI `31a50dc` chạy 2048 test khác xanh nhưng matrix thiếu/skew product contract mất 5117 ms dưới full-suite contention; cô lập chỉ 261–281 ms và Windows exact xanh. Giữ nguyên toàn bộ candidate/assertion/production code, đặt finite per-case ceiling POSIX 15 s và Windows 30 s. Đây là orchestration budget của test, không thay runtime timeout hay acceptance semantics | checklist M exact-head CI, `tests/cli/sea-bootstrap.test.ts` |

| C-71 | Nguồn release đã được duyệt ở C-21 nhưng **không có code nào chạy nó**, nên đường duy nhất tới được artifact vẫn là smoke fixture | `build-ffmpeg.mjs` giữ pin + luật build từ Phase M mà chưa bao giờ có driver; một danh sách nguồn được duyệt không sinh ra binary nào. `scripts/build-release-media.mjs` dựng ffmpeg/ffprobe 7.1.1 từ đúng các pin đó, hỏi linker xem kết quả còn phụ thuộc gì máy sạch không có, rồi ghi provenance nêu đích danh archive nguồn. `prepare-packaged-runtime --release-media` đọc lại record và **kiểm hai câu độc lập**: record nêu đúng tập nguồn được duyệt, và binary cạnh nó vẫn đúng bytes record đã đo. Xin release media **không phải** là quyền: không có source build thì opt-in fixture vẫn từ chối. x264 host trả `406` cho HTTP client của Node và `200` cho curl cùng URL ⇒ fallback transport thứ hai, digest vẫn là thứ quyết định bytes | checklist Phase D AC, `scripts/build-release-media.mjs`, `scripts/prepare-packaged-runtime.mjs`, `tests/build/release-media.test.ts` |

| C-72 | Packaged smoke ghi bằng chứng **sau** khi dọn dẹp, nên một lỗi persist cache xoá luôn report của cả lượt chạy | Lượt release thật hoàn tất 13 bước rồi mất sạch report + evidence bundle vì `cp` ném khi ghi cache về (thư mục ở smoke root đối đầu file thường ở cache root). Teardown nay chạy **sau** khi evidence đã nằm trên đĩa; write-back chịu lỗi **theo từng entry** và nêu đích danh entry đó trong report; lỗi evidence được **giữ lại** thay vì ném, nên lượt chạy vẫn báo cáo việc nó đã làm và vẫn exit ≠ 0. Chiều **seed** cache vẫn ném: đó là setup mà các bước sau khẳng định dựa lên, hỏng thì phải dừng chứ không báo cáo sau. Cộng thêm: `build-release-media` tự hỏi binary vừa ra khỏi compiler có mang đường checkout không, thay vì để `verify-artifact` phát hiện sau một runtime stage + archive + SEA build | checklist Phase D AC/M.6, `scripts/packaged-smoke/{run,environment}.mjs`, `scripts/build-release-media.mjs`, `tests/build/{packaged-smoke,release-media}.test.ts` |

> **Đính chính số lượng:** phụ lục hiện có **72 correction**. Đoạn tổng kết lịch sử ngay dưới đây giữ nguyên câu đếm cũ tới C-68; C-69–C-72 cũng không mở rộng requirement mà sửa false-negative của MCP stderr, full-suite test budget, việc C-21 chưa có đường thực thi, và việc teardown của harness xoá được bằng chứng của một lượt chạy đã xong.

**Không có món nào trong sáu mươi tám món này mở rộng requirement.** C-1 đổi *chỗ đặt code* (remote `ToolInvoker`: `mcp` → `cli`) theo hướng thắt boundary. C-6 đóng băng vocabulary của wire contract đã phát hành. C-8 giữ nguyên thứ tự cold boot và làm rõ source-dev. C-9 loại hai authority layout mâu thuẫn. C-10 thay một timeout không thể hoạt động với compiler đồng bộ bằng process boundary vốn đã được R6.11 yêu cầu. C-11 giữ nguyên SEA một-file/verified archive, nhưng tách stage tải code vì runtime thật không thể evaluate external native graph trước extraction. C-12 gỡ build/install metadata không có runtime caller và làm scanner phân biệt exact pinned Node baseline với payload VidCom thêm, nên gate purity vẫn chặt hơn chứ không có ID allowlist. C-13 đo bytes từ chính executable thay vì tin tool inject. C-14 bind mọi filesystem mutation vào cùng generation capability thay vì tin lexical path. C-15 khớp hai authority đã được requirement yêu cầu: job cho progress/terminal và workspace-operation cho recovery filesystem. C-16–C-20 sửa các giả định wiring, cache, process và phép đo bằng runtime evidence. C-21 giữ nguyên human supply-chain gate thay vì lén biến fixture CI thành nguồn release. C-22–C-23 làm rõ lifecycle capability/session và perimeter theo namespace. C-24/C-29/C-37/C-41 tiếp tục phân biệt mọi exact package-manager input có thể shared với release output bắt buộc một link. C-25 áp semantics quyền đúng theo filesystem đích mà không nới byte/path integrity. C-26 đưa spike proof về cùng process-identity authority production đã dùng, tránh vừa báo survivor giả vừa có nguy cơ kill PID mới. C-27/C-36/C-38 thay đổi scheduling test theo authority/chi phí runtime thật nhưng không giảm test coverage. C-28 giữ đúng thứ tự auth-before-routing đã chốt ở C-23 và tách khẳng định NoWorkspace khỏi route-presence leak. C-30 giữ degraded-proof trung thực thay vì đòi một capability đã bị workflow tắt. C-31/C-34/C-35 buộc probe, test cleanup và lock release về cùng authority identity/ownership fail-closed. C-32 làm scanner semantic hơn nhưng vẫn bắt secret material thật. C-33 thêm proof output và đóng mutable HyperFrames experiment, không thay render contract. C-39 tách mutation repair khỏi strict reporting policy. C-40 thêm parent-only lifecycle control để harness chạy đúng graceful stop đã có, không mở surface mới. C-42 đổi shell host khi có sẵn nhưng giữ nguyên .NET process-start authority và identity scheme. C-43 buộc repair có integrity diagnosis thật trước mutation. C-44 chỉ mở rộng bounded diagnostic để tìm root cause, không nới artifact acceptance. C-45 canonicalize duy nhất đường đi của smoke client, không nới production browser/import contract. C-46 chỉ cố định runtime của build tool đã pin/lock, không đổi SEA format hay bytes acceptance. C-47 làm network cut fail-fast thay vì loopback-hang, không cho thêm external egress. C-48 thay implementation injector Linux vì tool pin không xử lý được blob production-size; one-file SEA, fuse, active-resource byte verification và atomic publication vẫn giữ nguyên. C-49 làm baseline đã commit trở thành input bắt buộc có schema đóng, đúng với yêu cầu gate không được tự tắt. C-50 làm test dùng đúng lifecycle handle và process budget của host, không nới assertion hay artifact acceptance. C-51 chỉ loại orchestration trùng lặp trên PR; standalone heavy workflows và coverage ba OS vẫn giữ nguyên. C-52 giữ browser integration ở workflow đã cài exact Chrome, không giảm browser assertion hay process-supervision proof. C-53–C-68 tiếp tục đóng topology/runtime/MCP/evidence theo capability thật: không bootstrap side effect ở thin bridge, không nới timeout control-plane, không bỏ diagnostics khi output lớn, không bỏ deep hash để đạt performance, không retry mutation, và không coi source package là runtime closure của artifact.
