...
- **R2.14** — WHEN daemon **mất lease** giữa phiên (renew fail, bị takeover sau TTL) THEN nó SHALL từ chối **mọi** đường ghi với một mã lỗi rõ ràng, và SHALL **hoặc** re-acquire được lease **hoặc** tự dừng — gồm đóng listener. Hôm nay `onLeaseLost` chỉ revoke session và dừng background, listener vẫn mở và tiến trình vẫn sống (§1.4), nên cộng với R2.12 sẽ thành **hai daemon cùng sống** và cái không lease vẫn nhận request từ bridge. Phải có test: sau khi mất lease, một tool ghi qua bridge SHALL fail có mã, MUST NOT ghi được.
