...
> **Bản 12 — 2026-08-04; evidence đa nền tảng đóng 2026-08-05.** **Spike bác bỏ hai giả định của bản 11, thêm R6.6b-ii.** Bản 11 giả định process group là bao đóng đủ trên POSIX và chỉ Windows mới có vấn đề. Đo thật ([spike checklist-gate](../../../../spikes/phase-3-checklist-gate/README.md)): `chrome-headless-shell` tự tách process group, `kill(-pgid)` để sót 5 process; và sweep theo quan hệ cha-con báo *sạch* trong đúng lúc leak vì con bị reparent sang `pid 1`. Cả hai lỗi đều thuộc loại **PASS giả** — cách đo cũ báo thành công trong khi đang rò. R6.6b viết lại quanh capture-PID-rồi-kill; R6.6b-i mở rộng cho mọi nền tảng; R6.6b-ii ghi lại hai điều cấm. Remediation đã đo PASS 3/3 trên POSIX; W1/W2 tiếp tục PASS trong CI Linux/Windows và real render Windows ở run `30965814511`.
> **Bản 11 — 2026-08-04.** **Sửa một AC theo quyết định người dùng ở pha Design (DG-1).** R6.6b hạ từ *zero-survivor guarantee* xuống *awaited `taskkill /T /F` + verify sweep có giới hạn đã biết*, và thêm R6.6b-i ghi rõ giới hạn đó. Lý do: guarantee cũ chỉ thực hiện được bằng Win32 Job Object, mà production code bị khoá ở TypeScript/Node — Node không expose API này. Người dùng chọn giữ nguyên stack thay vì duyệt ngoại lệ native sidecar; zero-survivor chuyển sang Giai đoạn 4. Đây là **hạ guarantee có chủ đích, được ghi lại**, không phải Design tự nới. Mọi AC khác của R6 giữ nguyên.
...
| AC | Nội dung chốt |
|---|---|
| **R6.6b** *(sửa 2026-08-04, bản 12 — xem R6.6b-i, R6.6b-ii)* | Kill cả cây process tường minh, không dựa cascade pipe · **process group KHÔNG đủ trên bất kỳ nền tảng nào** (R6.6b-ii) · phải **capture PID cụ thể trong lúc process chạy**, rồi kill mọi process group đã ghi **và** mọi PID đã ghi; Windows `taskkill /T /F` phải được **await tới khi thoát** · chỉ chuyển `cancelled` sau khi **verify sweep** xác nhận mọi PID đã ghi đều chết · xoá work directory · xoá thất bại → `cleanupPending: true`, để recovery xử lý |
| **R6.6b-i** *(mới)* | Verify là **best-effort có giới hạn đã biết trên MỌI nền tảng**, không phải zero-survivor guarantee: một descendant sinh ra **sau** lượt capture cuối không nằm trong tập quan sát nên không bị verify. Hệ quả bắt buộc: (a) sweep lặp tới khi hai lượt liên tiếp cho tập survivor rỗng hoặc chạm `PROCESS_VERIFY_MAX_SWEEPS`; (b) sweep cạn mà còn survivor → `process_termination_unverified`, **MUST NOT** ghi `cancelled`; (c) containment của R6.7b là tầng phòng thủ thứ hai và **không** được coi là dư thừa; (d) zero-survivor guarantee bằng Job Object là **Giai đoạn 4**, không phải Phase 3 |
| **R6.6b-ii** *(mới, do spike bác bỏ giả định)* | Hai giả định của bản 11 đã bị đo bác bỏ, cả hai đều theo hướng nguy hiểm — xem [spike checklist-gate](../../../../spikes/phase-3-checklist-gate/README.md). (a) **Process group không phải bao đóng**: `chrome-headless-shell` tự đặt mình vào process group riêng, nên `kill(-pgid)` để sót 5 process Chromium còn sống. (b) **Sweep theo quan hệ cha-con báo sạch trong lúc đang leak**: process con được reparent sang `pid 1` khi cha chết, nên duyệt theo ppid trả về rỗng đúng lúc leak xảy ra. Vì vậy verify **MUST** probe từng PID đã ghi trực tiếp (`kill(pid, 0)` hoặc tương đương), **MUST NOT** suy từ quan hệ cha-con, và **MUST NOT** suy từ thành viên process group |
...
- **Sửa sau duyệt (bản 11, 2026-08-04)**: R6.6b hạ guarantee + thêm R6.6b-i, theo quyết định người dùng trên DG-1 của Design. Phê duyệt bản 10 **vẫn còn hiệu lực** cho R1–R13 còn lại; chỉ ô R6.6b/R6.6b-i mang trạng thái mới. Design bản 3 đã đồng bộ theo.
