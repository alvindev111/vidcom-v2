...
> **Bản 45 — 2026-08-05.** Vá cách đo K.15 sau khi final closeout CI run `30966729860` lộ một warm-scan outlier Windows 147.23 ms dù exact implementation commit trước đó đã xanh. Target vẫn là warm `< 100 ms`; trên shared CI, test đo ba warm scan liên tiếp bằng filesystem thật và dùng p50, đồng thời giữ toàn bộ samples trong assertion. Một lần scheduler/antivirus giật không còn quyết định release, còn regression ổn định (ít nhất hai trong ba sample vượt ngưỡng) vẫn đỏ. Không cache canonical path vĩnh viễn và không hạ ngưỡng bảo mật/performance.
...
> **Bản 4 — 2026-08-04; evidence đa nền tảng đóng 2026-08-05.** Đóng ba số chưa đo bằng [spike checklist-gate](../../../../spikes/phase-3-checklist-gate/README.md). Scan và `--at` PASS. Số thứ ba **bác bỏ giả định POSIX của bản 3**: `chrome-headless-shell` tự tách process group nên `kill(-pgid)` để sót 5 process, và sweep theo quan hệ cha-con báo *sạch* trong đúng lúc leak (con bị reparent sang `pid 1`). §5.9 viết lại thành thuật toán ba pha capture→kill→probe-theo-PID, đo PASS 3/3, p95 = 2 sweep; Goals bản 12 thêm R6.6b-ii. Kèm ba sửa §5.11 do spike `--at`: map theo timestamp không theo ordinal, validate range, dedupe. W1/W2 đã đóng bằng GitHub Actions run `30965814511`: Linux và Windows đều PASS, Windows dùng PowerShell CIM và real render kết thúc sạch.
...
- **Đã đóng bằng CI ngày 2026-08-05**: process-supervision run `30965814511` xác nhận Linux dùng `ps` (capture 3 PID/2 group, 2 sweep, 120.4 ms, 0 survivor), Windows dùng `powershell-cim` (capture 4 PID/1 group, 2 sweep, 1921.7 ms, 0 survivor; lượt lặp 1906.7 ms), và render thật Windows capture 7 PID/1 group, 2 sweep, 2298.6 ms, 0 survivor. Workflow giữ các phép đo này thành tín hiệu đỏ thường trực nếu nền tảng nào phá contract.
...
- **Scan target — sửa bản 3.** Bản 2 viết "< 500 ms / 100 dir, **chỉ stat**", nhưng §4.3.2 phân biệt `authored` với `invalid(composition)` bằng **parse `index.html`**, và `WorkspaceEntry` mang `sceneCount` — cũng phải parse. Stat không cho hai thứ đó, nên hai câu không thể cùng đúng. Quyết định người dùng: **giữ phân loại đầy đủ, sửa target**.
  ...
  Trên runner CI dùng chung, warm target được đo bằng **p50 của ba warm scan liên tiếp** sau cold scan, cùng một `WorkspaceFs`/scanner và cùng 100 thư mục thật; test phải giữ cả ba sample trong failure output. Target p50 vẫn **< 100 ms**. Đây chỉ loại một scheduling/antivirus outlier: nếu ít nhất hai sample vượt ngưỡng thì gate vẫn đỏ và phải quay lại Design, không tăng threshold hay cache canonical containment vĩnh viễn.
...
- **Notes / required revisions before implementation**:
  ...
  - **Ba số và matrix đa nền tảng đã đo xong** ([spike checklist-gate](../../../../spikes/phase-3-checklist-gate/README.md)). Hai cái PASS thẳng (scan, `--at`). Cái thứ ba **bác bỏ một giả định của bản 3**: `kill(-pgid)` để sót 5 process Chromium trên macOS vì Chromium tự tách process group, và cách đo cũ báo sạch trong lúc đang leak. §5.9 viết lại thành thuật toán ba pha capture→kill→probe; Goals bản 12 thêm R6.6b-ii ghi hai điều cấm. Remediation đo PASS 3/3; W1/W2 tiếp tục PASS trong CI Linux/Windows và real render Windows ở run `30965814511`.
