...
function guard(environment: Record<string, string | undefined>) {
  return new CompilerGuard({ esbuildBinaryPath: BINARY, environment: environment as NodeJS.ProcessEnv });
}
...
describe("compiler guard", () => {
  ...
  it("reports both when neither variable is set", async () => {
    const result = await guard({}).run(() => "unreachable", 1_000);
    expect(result.ok).toBe(false);
    if (result.ok) return;
  ...
  it("returns the value when the compiler answers in time", async () => {
    const result = await guard(configured()).run(() => "compiled", 1_000);
  ...
  it("turns a thrown compiler error into the same coded failure", async () => {
    const result = await guard(configured()).run(() => { throw new Error("esbuild exploded"); }, 1_000);
    expect(result.ok).toBe(false);
    if (result.ok) return;
    expect(result.error.code).toBe(ErrorCode.CompilerUnavailable);
  ...
  it("refuses to run without a timeout at all", async () => {
    ...
    await expect(guard(configured()).run(() => "x", 0)).rejects.toThrow(TypeError);
    await expect(guard(configured()).run(() => "x", -1)).rejects.toThrow(TypeError);
  });
});
